...
//			return true;
	...
	@Override
	public int findId(Classe obj) {
		Classe cla=new Classe();
		try
		{
			ResultSet result = this.connect.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
					ResultSet.CONCUR_READ_ONLY).executeQuery("SELECT * FROM classe WHERE nom_classe = '"+obj.getNom_classe()+"' AND id_niveau='"+obj.getId_niveau()+"'");
			if(result.first())
			{
				cla=new Classe(result.getInt(1),result.getString(2),result.getInt(3),result.getInt(4),result.getInt(5));
			}
			
			return cla.getId_classe();

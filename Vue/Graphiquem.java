...
public class Graphiquem implements ActionListener, MouseListener{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			Personne elevee = new Personne(id_selectionne,txtNom1_2.getText(),txtPrenom1_2.getText(),"Eleve");
			
			Inscription inscr = new Inscription(id_selectionne);
			int id9 = inscriptionDAO.findId(inscr);
			inscr = inscriptionDAO.find(id9);

			Classe classee = classeDAO.find(inscr.getId_classe());
			int id91 = classeDAO.findId(classee);
			classee = classeDAO.find(id91);

			Niveau niveauu = niveauDAO.find(classee.getId_niveau());
			int id92 = niveauDAO.findId(niveauu);
			niveauu = niveauDAO.find(id92);
			
			//Graphiquen gph = new Graphiquen(connect, elevee, inscr, classee, niveauu);
			Graphiquep gph = new Graphiquep(connect, elevee, classee, niveauu, inscr);
			gph.initialize();
		}
